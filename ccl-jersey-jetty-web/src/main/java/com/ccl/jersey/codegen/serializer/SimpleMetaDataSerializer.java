...
import com.ccl.jersey.codegen.LabelImpl;
import com.ccl.jersey.codegen.Uniques;
...
/**
 * MetaDataSerializer defines the Query type serialization logic for
 * MetaDataExporter. Subclass this class for customization.
 *
 * @author ccl
 */
public class SimpleMetaDataSerializer extends EntitySerializer {
     ...
     *
     * @param namingStrategy naming strategy for table to class and column to property
     * conversion
     * @param innerClassesForKeys wrap key properties into inner classes (default: false)
     * @param imports java user imports
     */
    @Inject
    public SimpleMetaDataSerializer(
    ...
    protected void constructorsForVariables(CodeWriter writer, EntityType model)
        ...
        writer.beginConstructor(new Parameter("variable", Types.STRING),
                new Parameter("schema", Types.STRING), new Parameter("table",
                        Types.STRING));
    ...
    @Override
    protected void constructorContent(CodeWriter writer, EntityType model)
            throws IOException {
        writer.line("addMetadata();");
    }

    @Override
    protected void introClassHeader(CodeWriter writer, EntityType model)
            throws IOException {
        Type queryType = typeMappings.getPathType(model, model, true);

        Label entityLabelAnnotation = model.getAnnotation(Label.class);
        String entityLabel = entityLabelAnnotation.value();
        model.addAnnotation(new LabelImpl(entityLabel + "查询"));
        ...
        writer.beginClass(queryType, new ClassType(category, entityPathType,
                model));
        writer.privateStaticFinal(Types.LONG_P, "serialVersionUID",
                String.valueOf(model.hashCode()));
    }
    ...
    protected String getAdditionalConstructorParameter(EntityType model) {
        ...
        if (model.getData().containsKey("schema")) {
            builder.append(", \"").append(model.getData().get("schema"))
                    .append("\"");
        } else {
            builder.append(", null");
        }
        builder.append(", \"").append(model.getData().get("table"))
                .append("\"");
    ...
    protected void introDefaultInstance(CodeWriter writer,
                                        EntityType entityType, String defaultName) throws IOException {
        String variableName = !defaultName.isEmpty() ? defaultName
        ...
        writer.publicStaticFinal(queryType, variableName,
                NEW + queryType.getSimpleName() + "(\"" + alias + "\")");
    }
    ...
    protected void introImports(CodeWriter writer, SerializerConfig config,
        ...
        Collection<ForeignKeyData> foreignKeys = (Collection<ForeignKeyData>) model
                .getData().get(ForeignKeyData.class);
        Collection<InverseForeignKeyData> inverseForeignKeys = (Collection<InverseForeignKeyData>) model
        ...
        if (!entityPathType.getPackage().equals(
                ColumnMetadata.class.getPackage())) {
    ...
    protected void writeUserImports(CodeWriter writer) throws IOException {
        ...
        for (String javaImport : imports) {
            // true if the character next to the dot is an upper case or if no
            // dot is found (-1+1=0) the first character
            boolean isClass = Character.isUpperCase(javaImport
                    .charAt(javaImport.lastIndexOf(".") + 1));
            if (isClass) {
    ...
    @Override
    protected void outro(EntityType model, CodeWriter writer)
            throws IOException {
        ...
        for (Property property : properties) {
            ...
            ColumnMetadata metadata = (ColumnMetadata) property.getData().get(
                    "COLUMN");
    ...
    protected void serializeProperties(EntityType model,
                                       SerializerConfig config, CodeWriter writer) throws IOException {
        Collection<PrimaryKeyData> primaryKeys = (Collection<PrimaryKeyData>) model
                .getData().get(PrimaryKeyData.class);
        Collection<ForeignKeyData> foreignKeys = (Collection<ForeignKeyData>) model
                .getData().get(ForeignKeyData.class);
        Collection<InverseForeignKeyData> inverseForeignKeys = (Collection<InverseForeignKeyData>) model
                .getData().get(InverseForeignKeyData.class);
        ...
        if (innerClassesForKeys) {
            Type primaryKeyType = new SimpleType(
                    namingStrategy.getPrimaryKeysClassName());
            Type foreignKeysType = new SimpleType(
                    namingStrategy.getForeignKeysClassName());
            ...
            if (foreignKeys != null || inverseForeignKeys != null) {
                ...
                if (inverseForeignKeys != null) {
                    serializeForeignKeys(model, writer, inverseForeignKeys,
                            true);
                }
                writer.end();
            }
            ...
            if (primaryKeys != null) {
                writer.publicFinal(primaryKeyType,
                        namingStrategy.getPrimaryKeysVariable(model), "new "
                                + primaryKeyType.getSimpleName() + "()");
            }
            if (foreignKeys != null || inverseForeignKeys != null) {
                writer.publicFinal(foreignKeysType,
                        namingStrategy.getForeignKeysVariable(model), "new "
                                + foreignKeysType.getSimpleName() + "()");
            }
    ...
    @Override
    protected void customField(EntityType model, Property field,
                               SerializerConfig config, CodeWriter writer) throws IOException {
        Type queryType = typeMappings
                .getPathType(field.getType(), model, false);
        String localRawName = writer.getRawName(field.getType());
        serialize(model, field, queryType, writer, "create"
    ...
    protected void serializePrimaryKeys(EntityType model, CodeWriter writer,
        ...
        for (PrimaryKeyData primaryKey : primaryKeys) {
            String fieldName = namingStrategy.getPropertyNameForPrimaryKey(
                    primaryKey.getName(), model);
    ...
    protected void serializeForeignKeys(EntityType model, CodeWriter writer,
                                        Collection<? extends KeyData> foreignKeys, boolean inverse)
            throws IOException {
        for (KeyData foreignKey : foreignKeys) {
            String fieldName;
            if (inverse) {
                fieldName = namingStrategy.getPropertyNameForInverseForeignKey(
                        foreignKey.getName(), model);
            } else {
                fieldName = namingStrategy.getPropertyNameForForeignKey(
                        foreignKey.getName(), model);
            }
            ...
            if (foreignKey.getForeignColumns().size() == 1) {
                value.append(namingStrategy.getPropertyName(foreignKey
                        .getForeignColumns().get(0), model));
                ...
                for (int i = 0; i < foreignKey.getForeignColumns().size(); i++) {
                    ...
                    local.append(namingStrategy.getPropertyName(foreignKey
                            .getForeignColumns().get(i), model));
                    foreign.append("\"" + foreignKey.getParentColumns().get(i)
                            + "\"");
                }
                value.append("Arrays.asList(" + local + "), Arrays.asList("
                        + foreign + ")");
            }
            value.append(")");
